@props(['label', 'name', 'type' => 'text', 'wire' => true, 'col' => false, 'showerror' => true, 'mb' => '' ,'value' => ''])
...
<x-bootstrap.form.group class="{{ $mb?:'' }}" :col="$col">
    ...
    @if ($col)
        <div class="col-sm-12 col-md-7">
            <input id="{{ $name }}"  type="{{ $type }}" {!! $attributes->merge(['class' => "form-control $errorClass"]) !!} {!! $wire ? ( $type != "file" ?  "wire:model='$name'" : "wire:model='$name'") : "name='$name' value='$value'" !!}/>
            @if ($showerror)
                @error($name)
